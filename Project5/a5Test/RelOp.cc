...
//    cout<<"Enter DuplicateRemoval::Run" << endl;
    //Construct parameters and send them to main worker
...
//    cout<<"Exit DuplicateRemoval::Run" << endl;
}

void* DuplicateRemovalWorker (void* arg) {
//    cout<<"Enter DuplicateRemovalWorker" << endl;
...
//    cout<<"Exit DuplicateRemovalWorker" << endl;
    return NULL;
}
...
void* JoinWorker(void* arg) {
    ...
    if (leftOrder.numAtts > 0 && rightOrder.numAtts > 0) {
        ...
        JoinWorker_BlockNested_DBFile(joinArg);
    }
    return NULL;
}
...
void JoinWorker_BlockNested_DBFile(JoinArg* joinArg) {
    ...
    sprintf(metaFileName, "BlockNestedTemp%d.bin.meta", pthread_self());
    //Create and Open the file
    tempFile.Create(fileName, heap, NULL);
    // tempFile.Open(fileName);
    Record record;
    while (joinArg->inPipeR->Remove(&record) == 1) {
        Record temp;
        temp.Consume(&record);
        tempFile.Add(temp);
    }
    ...
    ComparisonEngine comparisonEngine;
    while (joinArg->inPipeL->Remove(&record1) == 1) {
        tempFile.MoveFirst();
        while (tempFile.GetNext(record2) == 1) {
            Record *temp1 = new Record();
            Record *temp2 = new Record();
            temp1->Copy(&record1);
            temp2->Copy(&record2);
            if (comparisonEngine.Compare(temp1, temp2, joinArg->literal, joinArg->selOp)) {            
                JoinWorker_AddMergedRecord(temp1, temp2, joinArg->outPipe);
            }
        }
    }
    tempFile.Close();
...
void JoinWorker_BlockNested_Vector(JoinArg* joinArg) {
    vector<Record*> vl, vr;
    Record record1, record2;
    while (joinArg->inPipeL->Remove(&record1) == 1) {
        Record* temp = new Record();
        temp->Consume(&record1);
        vl.push_back(temp);
    }
    while (joinArg->inPipeR->Remove(&record2) == 1) {
        Record* temp = new Record();
    ...
    for (int i = 0; i < vl.size(); i++) {
        for (int j = 0; j < vr.size(); j++) {
            if (comparisonEngine.Compare(vl[i], vr[j], joinArg->literal, joinArg->selOp)) {
                JoinWorker_AddMergedRecord(vl[i], vr[j], joinArg->outPipe);
            }    
        }
    }
